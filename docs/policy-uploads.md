...
- `node --test tests/policy.test.mjs`: 12 tests covering real generated PDF/DOCX extraction, validation, chunk overlap, source isolation, expiry, escalation, prompt safeguards, response shape, citation validation, and upload handling. Claude is mocked and no paid requests occur. The test loader uses Node's `registerHooks` (Node 22.15+ or 24+).
- `npm run lint`
- `npm run build`
...
5. Select **Use default handbook** and verify the default name and handbook answers return.
